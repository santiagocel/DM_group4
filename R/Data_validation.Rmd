...
if (!any(is.na(missing_values)) && 
    length(unique(customer_data$customer_id)) == nrow(customer_data) &&
    ...
    all(customer$date_of_birth %in% valid_ages) &&
    all(grepl("^\\d{4}-\\d{2}-\\d{2}$", customer$date_of_birth)) &&
    all(grepl("^\\S+@\\S+\\.\\S+$", customer$email)) &&
    all(sapply(customer$password_hash, is.character)) &&
    all(customer$membership %in% valid_membership)){
  print("Data is valid.")
...
```{r Data Validation for Quality and Integrity : customer address}
# Check Primary Key
...
for (field in mandatory_fields) {
  if (any(is.na(customer_address_data[[field]]) | customer_address_data[[field]] == "")) {
    print(paste("Missing or empty values detected in", field, "."))
  }
}

# Check length
if (any(nchar(customer_address_data$zipcode) > 10)) {
  print("zipcode exceeds 10 characters.")
}

if (any(nchar(customer_address_data$country) > 20)) {
  print("country exceeds 20 characters.")
}

if (any(nchar(customer_address_data$state) > 50)) {
  print("state exceeds 50 characters.")
}

if (any(nchar(customer_address_data$city) > 50)) {
  print("city exceeds 50 characters.")
}

if (any(nchar(customer_address_data$street) > 255)) {
  print("street exceeds 255 characters.")
}

# If no errors are found, print a message indicating that the data is valid
if (!any(is.na(mandatory_fields)) && 
    all(customer_address_data$customer_id %in% customer$customer_id) &&
    all(nchar(customer_address_data$zipcode) > 10) &&
    all(nchar(customer_address_data$country) > 20) &&
    all(nchar(customer_address_data$state) > 50) &&
    all(nchar(customer_address_data$city) > 50) &&
    all(nchar(customer_address_data$street) > 255)){
...
# Check Primary Key
if (length(unique(orders_data$orders_id)) != nrow(orders_data)) {
  print("Order ID is not unique.")
}

# Referential Integrity
if (!all(orders_data$customer_id %in% customer_data$customer_id)) {
...
if (!any(is.na(na_order)) &&
    ...
    all(orders_data$customer_id %in% customer_data$customer_id)))) {
...
```{r Data Validation for Quality and Integrity : Order Detail}
# Check Primary key
if (length(unique(order_details_data$order_details_id)) != nrow(order_details_data)) {
...
if (!all(order_details_data$product_id %in% products_data$product_id)) {
...
```{r Data Validation for Quality and Integrity : Transaction}
# Check primary key
if (length(unique(transactions_data$transactions_id)) != nrow(transactions_data)) {
...
if (!all(transactions_data$payment_method %in% valid_payment_methods)) {
  stop("Invalid payment_method detected.")
}
if (!all(transactions_data$transaction_status %in% valid_transaction_statuses)) {
  stop("Invalid transaction_status detected.")
}

# Check business rule: If order is cancelled, there should be no transaction
cancelled_orders <- orders_data$order_id[orders$order_status == 'Cancelled']
...
```{r Data Validation for Quality and Integrity : Delivery}
# Check primary key
...
if (!all(delivery_data$customer_address_id %in% customer_address_data$customer_address_id)) {
...
if (any(delivery_data$delivery_status == 'Failed' & (is.na(delivery_data$delivery_start_date) | is.na(delivery_data$delivery_end_date) | as.numeric(difftime(delivery_data$delivery_end_date, delivery$delivery_start_date, units = "days")) <= 30))) {
  stop("'Failed' deliveries have invalid dates or the duration is not more than 30 days.")
}

```

```{r Data Validation for Quality and Integrity : Category}
...
```{r Data Validation for Quality and Integrity : Product}
# Check the primary key
...
```{r Data Validation for Quality and Integrity : Supplier}
#Check the primary key
...
# Check length
if (any(nchar(supplier_data$supplier_zipcode) > 10)) {
  print("zipcode exceeds 10 characters.")
}
...
```{r Data Validation for Quality and Integrity : Supply}
...
```{r Data Validation for Quality and Integrity : Ads}
# Check the primary key
...
ads$ad_start_date <- as.Date(ads_data$ad_start_date)
ads$ad_end_date <- as.Date(ads_data$ad_end_date)
